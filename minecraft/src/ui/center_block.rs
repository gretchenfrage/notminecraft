...
fn y_translate<I: UiBlock>(size: Extent2<f32>, inner: &I) -> f32 {
    (size.h - inner.height()) / 2.0
}

impl<I: UiBlock> UiVCenterBlock<I> {
    pub fn new<F>(
        create_inner: F,
        size: Extent2<f32>,
        scale: f32,
    ...
    {
        let inner = create_inner(scale);
        let y_translate = y_translate(size, &inner);
...
impl<I: UiBlock> UiBlock for UiVCenterBlock<I> {
    type WidthChanged = False;
    type HeightChanged = False;
    ...
    )
    {
        ...
        let (
            _,
            inner_height_changed,
        ) = self.inner.set_scale(renderer, self.scale);
        if inner_height_changed.into() {
            self.y_translate = y_translate(self.size, &self.inner);
        }
...
impl<I: UiBlock> UiBlockSetWidth for UiVCenterBlock<I> {
    fn set_width(&mut self, _: &Renderer, width: f32) {
        self.size.w = width;
...
impl<I: UiBlock> UiBlockSetHeight for UiVCenterBlock<I> {
    fn set_height(&mut self, _: &Renderer, height: f32) {
        self.size.h = height;
