...
/// effects queue. After calling any methods on `ChunkMgr` that takes
/// `&mut self`, events should be taken from `ChunkMgr.effects` and
...
pub struct ChunkMgr {
    ...
    add_chunk_mgr: PerClientConn<ClientAddChunkMgr>,
...
/// Interface for `ChunkMgr` to ask whether loaded chunks are fully saved.
pub trait IsChunkSaved {
...
impl ChunkMgr {
    ...
    pub fn add_client(&mut self, ck: ClientConnKey) {
        ...
        self.add_chunk_mgr.insert(ck, ClientAddChunkMgr::new(&self.chunks));
    ...
    pub fn decr_load_request_count<S>(
        &mut self,
        cc: Vec3<i64>,
        conn_states: &ConnStates,
        saved: &S,
        ...
        S: GetIsSaved,
    {
        if let Some(ci) = self.chunks.getter().get(cc) {
            ...
            *count -= 1;
            ...
            if *count == 0 && !saved.is_chunk_saved(cc, ci) {
    ...
    /// Call upon a chunk being saved.
    pub fn on_saved(&mut self, cc: Vec3<i64>, ci: usize, conn_states: &ConnStates) {
